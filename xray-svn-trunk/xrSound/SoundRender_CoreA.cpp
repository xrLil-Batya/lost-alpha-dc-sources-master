...
#ifndef _EDITOR
	// utak3r: try to get the interface to EAX functions, no matter what OpenAL reported while enumerating:
	bEAX = (((eaxSet = (EAXSet)alGetProcAddress((const ALchar*)"EAXSet")) != NULL) &&
			((eaxGet = (EAXGet)alGetProcAddress((const ALchar*)"EAXGet")) != NULL));
#else
