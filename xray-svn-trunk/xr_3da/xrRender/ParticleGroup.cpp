...
BOOL CPGDef::Load(IReader& F)
{
	...
	bool bTimeLimit = (m_fTimeLimit > 0.0f);
...
void CParticleGroup::SItem::StartRelatedChild(CParticleEffect* emitter, LPCSTR eff_name, PAPI::Particle& m)
{
	...
	M.c.set					(p);
	C->Set_InitialPosition(m_InitialPosition);
	C->Play					();
...
void CParticleGroup::SItem::Play()
{
	...
	if (E) { E->Set_InitialPosition(m_InitialPosition); E->Play(); }
}
			...
			if (E){
				...
				if (E->IsPlaying()){
					...
					if (def.m_Flags.is(CPGDef::SEffect::flOnPlayChildRewind)){
						E->Set_InitialPosition(m_InitialPosition);
						E->Play	();
					}
				}
			}
...
//tatarinrafa:Added particle_distance contorling option

void CParticleGroup::OnFrame(u32 u_dt)
{
	Fvector					temp_pos;
	float distance_from_camera = temp_pos.sub(Device.vCameraPosition, m_InitialPosition).magnitude();
	//Msg("CParticleGroup distance %f", distance2);
	if (GetHudMode()){ distance_from_camera = 0; }
	if (m_InitialPosition.x == 0 && m_InitialPosition.y == 0 && m_InitialPosition.z == 0) { distance_from_camera = 0; }
	//{ Msg("Debug rafa!: %s->m_InitialPosition is nill", Name().c_str()); distance_from_camera = 0; };
#pragma todo("tatarinrafa: My particle distance tweak has some problemm when compiling Mixed bins")

	if (distance_from_camera < ps_r2_particle_distance || ps_r2_particle_distance >= 990){
...
{
	//Msg("CParticleGroup::UpdateParent");
	m_InitialPosition		= m.c;
	for (SItemVecIt i_it = items.begin(); i_it != items.end(); i_it++) {
		i_it->UpdateParent(m, velocity, bXFORM);
		i_it->m_InitialPosition = m_InitialPosition;
	}
}
