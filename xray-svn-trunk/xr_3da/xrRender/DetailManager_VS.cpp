...
//					C.mad					(c_lmap,Instance.c_rgb);
			...
			// Clean up
#if RENDER!=R_R1 //thx to K.D.
			if ((ps_r2_ls_flags.test(R2FLAG_SUN_DETAILS) && (RImplementation.PHASE_SMAP == RImplementation.phase))
				|| (ps_r2_ls_flags.test(R2FLAG_SUN_DETAILS) && (RImplementation.PHASE_NORMAL == RImplementation.phase) && (!RImplementation.is_sun_active()))
