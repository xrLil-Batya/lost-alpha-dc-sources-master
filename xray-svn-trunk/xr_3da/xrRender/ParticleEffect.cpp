...
void CParticleEffect::OnFrame(u32 frame_dt)
{
	...
	if (m_InitialPosition.x == 0 && m_InitialPosition.y == 0 && m_InitialPosition.z == 0){ distance_from_camera = 0; } // Msg("Debug rafa!: %s->m_InitialPosition is nill", Name().c_str()); }

	if (distance_from_camera < ps_r2_particle_distance || ps_r2_particle_distance >= 990){

		if (m_Def && m_RT_Flags.is(flRT_Playing)){
			m_MemDT += frame_dt;
			//Msg("OnFrame Distance from camera %s, %f, %f", Name().c_str(), distance_from_camera);
...
void CParticleEffect::Render(float )
{
	...
	if (p_cnt>0){
#else
	if (p_cnt>0 && (distance_from_camera < ps_r2_particle_distance || ps_r2_particle_distance >= 990)){
#endif
		if (m_Def&&m_Def->m_Flags.is(CPEDef::dfSprite)){
			//Msg("Render Distance from camera %s, %f", Name().c_str(), distance_from_camera);
