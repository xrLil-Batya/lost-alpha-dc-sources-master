...
u32			ps_r_smap_size			=	2560; //default
...
float		ps_r2_tonemap_amount		= 2.f;				// r2-only
float		ps_r2_ls_bloom_kernel_g		= 3.f;	// was 3.3f			// r2-only
float		ps_r2_ls_bloom_kernel_b		= .5f;		// was 0.7f		// r2-only
...
float		ps_r2_sun_near_border		= 0.75f;			// 0.99f // 1.0f
...
float		ps_r2_dof_sky				= 30.0f;				//	distance to sky
...
int			ps_r2_fxaa = 0;//tatarinrafa: Radium mod FXAA
//- Mad Max
float		ps_r2_gloss_factor			= 4.0f;
//- Mad Max
float		ps_r2_particle_distance		= 75.0f;				// 75.0f
...
// Common
	...
	CMD4(CCC_Float,		"r__geometry_lod",		&ps_r__LOD,					0.1f,	3.0f /*1.2f*/	);
//.	CMD4(CCC_Float,		"r__geometry_lod_pow",	&ps_r__LOD_Power,			0,		2		);

	CMD4(CCC_detail_radius,	"r__detail_radius",	&ps_r__Detail_radius,		30,	150	);
	CMD4(CCC_Float,		"r__detail_density",	&ps_r__Detail_density,		.18f,	1.0f	);
	...
	// gr1ph00n, it doesn't work properly. --skyloader: i added swing params to weather
	// It writes trash to user.ltx. I'm turning it off for now.
	//CMD4(CCC_SwingFactorsDetailsManager, "r__swing_factor0", 0, FLT_MIN, FLT_MAX);
	//CMD4(CCC_SwingFactorsDetailsManager, "r__swing_factor1", 1, FLT_MIN, FLT_MAX);
	...
	CMD4(CCC_Float,		"r2_tonemap_middlegray",&ps_r2_tonemap_middlegray,	0.15f,	0.8f	);
	...
	CMD4(CCC_Float,		"r2_particle_distance",	&ps_r2_particle_distance,	0.0f,	1000.f	);
...
//	CMD4(CCC_Float,		"r2_sun_lumscale",		&ps_r2_sun_lumscale,		-1.0,	+3.0	); //skyloader: moved to weather
...
//Separate values for steep parallax
	...
	CMD4( CCC_Dof,		"r2_dof",		&ps_r2_dof, tw_min, tw_max); //skyloader: moved to weather
	CMD4( CCC_DofNear,	"r2_dof_near",	&ps_r2_dof.x, tw_min.x, tw_max.x); //skyloader: moved to weather
	CMD4( CCC_DofFocus,	"r2_dof_focus", &ps_r2_dof.y, tw_min.y, tw_max.y); //skyloader: moved to weather
	CMD4( CCC_DofFar,	"r2_dof_far",	&ps_r2_dof.z, tw_min.z, tw_max.z); //skyloader: moved to weather
	...
	CMD3(CCC_Mask,		"r4_wireframe",					&ps_r2_ls_flags_ext,		R2FLAGEXT_WIREFRAME);//Need restart //skyloader: for tessellation test
	...
	CMD3(CCC_Token,		"r3_backbuffers_count",			&ps_r3_backbuffers_count,	qbbuffer_token	);
	CMD4(CCC_Integer,	"r2_fxaa",						&ps_r2_fxaa,				0, 1		);//tatarinrafa: Radium mod FXAA


//	CMD3(CCC_Mask,			"r3_volumetric_smoke",			&ps_r2_ls_flags,			R3FLAG_VOLUMETRIC_SMOKE); //skyloader: not used in the game, plus eat many fps in game :)
