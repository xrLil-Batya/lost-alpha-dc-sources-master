...
void CSkeletonX_ext::_FillVerticesHW1W(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size, Fvisual* V, u16* indices, CBoneData::FacesVec& faces)
{
	vertHW_1W*			vertices;
#if defined(USE_DX10) || defined(USE_DX11)
	vertHW_1W*	pVOriginal;
	pVOriginal	= xr_alloc<vertHW_1W>(V->vCount);
	vertices 	= pVOriginal;
...
#if defined(USE_DX10) || defined(USE_DX11)
	R_CHK(dx10BufferUtils::CreateVertexBuffer(&V->p_rm_Vertices, pVOriginal, V->vCount*vStride));
	HW.stats_manager.increment_stats_vb		( V->p_rm_Vertices);
	xr_free(pVOriginal);
#else	//	USE_DX10
	CHK_DX				(V->p_rm_Vertices->Unlock());
...
void CSkeletonX_ext::_FillVerticesHW2W(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size, Fvisual* V, u16* indices, CBoneData::FacesVec& faces)
{
	vertHW_2W*			vertices;
#if defined(USE_DX10) || defined(USE_DX11)
	vertHW_2W*	pVOriginal;
	pVOriginal	= xr_alloc<vertHW_2W>(V->vCount);
	vertices 	= pVOriginal;
...
void CSkeletonX_ext::_FillVerticesHW3W(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size, Fvisual* V, u16* indices, CBoneData::FacesVec& faces)
{
	vertHW_3W*			vertices;
#if defined(USE_DX10) || defined(USE_DX11)
	vertHW_3W*	pVOriginal;
	pVOriginal	= xr_alloc<vertHW_3W>(V->vCount);
	vertices 	= pVOriginal;
	...
	for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end(); ++it)
	{
		...
		for (u32 k=0; k<3; k++)
		{
			...
			Fmatrix& xform0			= Parent->LL_GetBoneInstance(F.bone_id[k][0]).mRenderTransform; 
			Fmatrix& xform1			= Parent->LL_GetBoneInstance(F.bone_id[k][1]).mRenderTransform;
			Fmatrix& xform2			= Parent->LL_GetBoneInstance(F.bone_id[k][2]).mRenderTransform; 
			vert.get_pos			(F.vert[k]);
			xform0.transform_tiny(P0,F.vert[k]);
			xform1.transform_tiny(P1,F.vert[k]);
			...
			P0.mul(F.weight[k][0]);
			P1.mul(F.weight[k][1]);
			P2.mul(1-F.weight[k][0]-F.weight[k][1]);
			p[k] = P0;
			p[k].add(P1);
			p[k].add(P2);
		}
...
void CSkeletonX_ext::_FillVerticesHW4W(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size, Fvisual* V, u16* indices, CBoneData::FacesVec& faces)
{
	vertHW_4W*			vertices;
#if defined(USE_DX10) || defined(USE_DX11)
	vertHW_4W*	pVOriginal;
	pVOriginal	= xr_alloc<vertHW_4W>(V->vCount);
	vertices 	= pVOriginal;
	...
	for (CBoneData::FacesVecIt it=faces.begin(); it!=faces.end(); ++it)
	{
		...
		for (u32 k=0; k<3; k++)
		{
			...
			for (u16 i=0; i<4; ++i)
			{
				F.bone_id[k][i]		= vert.get_bone(i);
				Fmatrix& xform	= Parent->LL_GetBoneInstance(F.bone_id[k][i]).mRenderTransform;
				xform.transform_tiny(P[i],F.vert[k]);
			}
			...
			for (int j=0; j<3; ++j)
				P[j].mul(F.weight[k][j]);

			P[3].mul(1-F.weight[k][0]-F.weight[k][1]-F.weight[k][2]);
...
void CSkeletonX_ext::_FillVertices(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size, Fvisual* V, u16 bone_id, u32 iBase, u32 iCount)
{
	...
	case RM_SKINNING_SOFT:
		...
		else {VERIFY( !!(*Vertices4W) );_FillVerticesSoft4W		(view,wm,normal,size,indices+iBase,*faces);}
#if !defined(USE_DX10) && !defined(USE_DX11)	//	Don't use hardware buffers in DX10 since we can't read them
		break;
	case RM_SINGLE:
