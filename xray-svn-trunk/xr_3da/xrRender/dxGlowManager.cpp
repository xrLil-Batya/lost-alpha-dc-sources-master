...
void CGlowManager::add	(ref_glow G_)
{
	...
	if (G->dwFrame == RDEVICE.dwFrame)		return;

	int frame_div = RDEVICE.dwFrame - G->dwFrame;
	if (frame_div > 30) G->fade	= 0.f; //skyloader: hide glow
	...
	float	range = RDEVICE.vCameraPosition.distance_to_sqr	(G->spatial.sphere.P);
	if (range < dlim2) 
	{
		// 2. Use result of test
		if (G->bTestResult) {
			G->fade -= dt*FADE_SCALE_DOWN;
			if (G->fade<1.f)		G->fade = 1.f;
		} else {
...
{
	const Fvector& T 	= RDEVICE.vCameraTop;
	const Fvector& R 	= RDEVICE.vCameraRight;
	Fvector		Vr, Vt;
	Vr.mul 		(R,r);
	Vt.mul		(T,r);
