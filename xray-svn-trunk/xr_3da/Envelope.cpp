...
extern float evalEnvelopeAngle( CEnvelope *env, float time );
float CEnvelope::EvaluateAngle(float time){
	return angle_normalize_signed(evalEnvelopeAngle(this, time));
}
