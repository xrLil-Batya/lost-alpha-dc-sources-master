...
CTimer MainThreadTimer, Thread1Timer, Thread2Timer;
...
volatile u32	mt_Thread_marker2		= 0x12345678;
bool thread1ready, thread2ready;
void 			mt_Thread	(void *ptr)	{
	while (true) {
		...
		Device.Statistic->obects = Device.seqParallel.size();

		for (u32 pit = 0; pit < Device.seqParallel.size(); pit++){
			Device.seqParallel[pit]();
		}
		Device.seqParallel.clear_not_free	();
		//Msg("Auxilary Thread1 seqParallel time = %f", Thread1Timer.GetElapsed_sec()*1000.f);
		Device.seqFrameMT.Process(rp_Frame);

		Device.Statistic->Thread1Time = Thread1Timer.GetElapsed_sec()*1000.f;
		//Msg("Auxilary Thread1 time = %f, obects computed %i", Thread1Timer.GetElapsed_sec()*1000.f, Device.Statistic->obects);
		thread1ready = true;

		// now we give control to device - signals that we are ended our work
		// wait for device signal to continue - to start again
...
void 			mt_Thread2(void *ptr)	{

	while (true) {
		// waiting for Device permission to execute
		Device.mt_csEnter2.Enter();

		if (Device.mt_bMustExit2) {
			Device.mt_bMustExit2 = FALSE;				// Important!!!
			Device.mt_csEnter2.Leave();					// Important!!!
			return;
		}
		// we are ok to execute
		Thread2Timer.Start();
		thread2ready = false;
		mt_Thread_marker2 = Device.dwFrame;

		Device.Statistic->Thread2Time = Thread2Timer.GetElapsed_sec()*1000.f;
		//Msg("Auxilary Thread2 time = %f, %u", Device.Statistic->Thread2Time);
		thread2ready = true;

		// now we give control to device - signals that we are ended our work
		// wait for device signal to continue - to start again
		// returns sync signal to device
		Device.mt_csEnter2.Leave();
		Device.mt_csLeave2.Enter();
		Device.mt_csLeave2.Leave();
	}
}
...
void CRenderDevice::on_idle		()
{
	...
	if (b_is_Active && Begin())
	{
		...
		if (psDeviceFlags.test(rsCameraPos) || psDeviceFlags.test(rsStatistic) || show_FPS_only == TRUE || show_engine_timers == TRUE || Statistic->errors.size())
			Statistic->Show();
		End();
	}
	...
	// Suspend Main Tread, if threads need more time
	//Msg("%2.2fms = Render Time", Statistic->RenderTOTAL.result);
	
	if (!thread1ready) { 
		Statistic->MainThreadSuspendCounter += 1;
	//	if (!thread1ready){ Msg("Auxilary Thread1 was not ready"); Msg("(thread sync!)Suspend Main Thread; cur main thread time = %f", MainThreadTimer.GetElapsed_sec()*1000.f); }
	//	if (!thread2ready){ Msg("Auxilary Thread2 was not ready"); Msg("(thread sync!)Suspend Main Thread; cur main thread time = %f", MainThreadTimer.GetElapsed_sec()*1000.f); }
	}
	while (!thread1ready){
	...
	//Msg("Main Thread time = %f", Statistic->MainThreadTime);
}
