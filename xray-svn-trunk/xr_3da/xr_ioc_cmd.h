...
class ENGINE_API	CCC_FloatArray : public IConsole_Command
{
protected:
	float*			values;
	float			min, max;
	int				count;
public:
	CCC_FloatArray(LPCSTR N, float* V, u32 _count, float _min = 0, float _max = 1) :
		IConsole_Command(N),
		values(V),
		min(_min),
		max(_max),
		count(_count)
	{};
	const float	GetValue(u32 idx) const { return values[idx]; };
	void	GetBounds(float& fmin, float& fmax) const { fmin = min; fmax = max; }

	virtual void	Execute(LPCSTR args)
	{
		if (_GetItemCount(args, ' ') != count)
			InvalidSyntax();

		for (int i = 0; i < count; i++)
		{
			string64 buffer = { 0 };
			_GetItem(args, i, buffer, sizeof(buffer), ' ', "0", true);
	...
	virtual void	Status(TStatus& S)
	{
		for (int i = 0; i < count; i++)
		{
			xr_sprintf(S, sizeof(S), "%s %3.5f", S, values + i);
		}
		while (xr_strlen(S) && ('0' == S[xr_strlen(S) - 1]))	S[xr_strlen(S) - 1] = 0;
	}
	virtual void	Info(TInfo& I)
	{
		xr_sprintf(I, sizeof(I), "%d float values in range [%3.3f,%3.3f]", count, min, max);
	}
	virtual void	fill_tips(vecTips& tips, u32 mode)
	{
		TStatus  str = { 0 };
		for (int i = 0; i < count; i++)
		{
			xr_sprintf(str, sizeof(str), "%s%3.5f ", str, values + i);
		}
		xr_sprintf(str, sizeof(str), "%s (current)  [%3.3f,%3.3f]", str, min, max);
		tips.push_back(str);
		IConsole_Command::fill_tips(tips, mode);
	}
...
:
	...
	virtual void	Execute	(LPCSTR args)
	{
		...
		if ((3 != sscanf(args, "%f,%f,%f", &v.x, &v.y, &v.z)) &&
			(3 != sscanf(args, "(%f, %f, %f)", &v.x, &v.y, &v.z)))
		{
			InvalidSyntax();
			return;
		}
