...
//				Log("WARNING: Material in material found while bullet tracing. Incorrect behaviour of shooting is possible.");
	...
	*hit_res = bullet->hit_param; //default param
	...
	//skyloader: the flight of bullets is now dependent on the protection of stalkers. Now, the bullet will not penetrate through the strong armor, and the bullet will ricochet.
	if ( CAI_Stalker* stalker = smart_cast<CAI_Stalker *>(R.O))
	{
		float BoneArmor = stalker->GetBoneHitProtections()->getBoneArmor(R.element); 
...
*/
	...
	if ( (f < ricoshet_factor) && !mtl->Flags.test(SGameMtl::flNoRicoshet) && bullet->flags.allow_ricochet )	
	{
		...
		// Msg("Ricochet, scale = %.2f, ricoshet_factor = %.2f", scale, ricoshet_factor);
