...
public:
	...
	virtual u32						ef_detector_type	() const;
#ifdef XRGAME_EXPORTS
	// spawn supplies from given raw INI string
	virtual void					spawn_supplies		(LPCSTR);
	// spawn supplies from object's custom data
	virtual void					spawn_supplies		();
	// spawn alife object as a child of this object
			CSE_Abstract*			spawn_item			(LPCSTR section);
	// spawn supply item with given parameters
			void					spawn_item			(LPCSTR sect, int count, float prob, float cond, bool scope, bool launcher, bool silencer);
	// spawn single item
			CSE_Abstract*			spawn_item			(LPCSTR sect, float cond, bool scope, bool launcher, bool silencer);
