...
//	Msg("To Slot %s[%d]", *pIItem->object().cName(), pIItem->object().ID());
	...
	if (pIItem->GetSlot() == HELMET_SLOT || pIItem->GetSlot() == PNV_SLOT){
		...
		if (itemformoutfitslot)
		{
			...
			if (outfit)
			{
				if (pIItem->GetSlot() == HELMET_SLOT && outfit->block_helmet_slot == 1){
					Ruck(pIItem);
					return false;
				}

				if (pIItem->GetSlot() == PNV_SLOT && outfit->block_pnv_slot == 1){
					Ruck(pIItem);
					return false;
				}
			}
...
void CInventory::RepackBelt( PIItem pIItem )
{
	...
	{
		CInventoryItem* invAmmoObj = (*it);
		if( invAmmoObj->m_eItemPlace == eItemPlaceBelt && invAmmoObj->object().cNameSect()== pIItem->object().cNameSect() ) //Is on belt, is the same type;
		{
...
{
#pragma todo("tatarinrafa: что-то вызывает эту функцию по сто раз без каких либо причин|something calls this function for no reason numerous times")
	//Msg("CInventory::Activate begin");
	//Если каким-то боком нам попался деетктор, то ретун, так как, у детектора есть свая сисетма доставания
	...
	//Msg("CInventory::Activate 2 slot = %u", slot);
	if (slot != NO_ACTIVE_SLOT) {
		CInventoryItem* invitemfromslot = ItemFromSlot(slot);

		if (slot < SLOTS_TOTAL){
			if (CurrentDetectorInHand && invitemfromslot && !invitemfromslot->TrueFalseSingleHand()){
				//Msg("CInventory::Activate");
				CurrentDetectorInHand->ToggleDetector(false);
				Setneedtoactivateweapon(slot + 1);
				Msg("Detector in hands, Setneedtoactivateweapon");
				return false;
			}
		}
	}
	//Msg("CInventory::Activate %u", slot);
	...
	if(m_iActiveSlot == slot && m_iActiveSlot != NO_ACTIVE_SLOT && m_slots[m_iActiveSlot].m_pIItem && m_slots[m_iActiveSlot].m_pIItem->cast_hud_item())
	{
		m_slots[m_iActiveSlot].m_pIItem->cast_hud_item()->Activate();
	}
	...
	case kWPN_6:
       {
		...
		//   Msg("cmd %u, flags %u", cmd, flags);
			if(flags&CMD_START && !m_bHandsOnly)
			{
				...
				{
					if (IsGameTypeSingle()){
					//	Msg("b_send_event = Activate(NO_ACTIVE_SLOT);");
