				   ...
				   bool SendHit)
{
	flags._storage			= 0;
	bullet_pos 				= position;
	speed = max_speed		= starting_speed * cartridge.param_s.kSpeed;
	VERIFY					(speed > 0.f);

	start_position			= position;
	start_velocity.mul		(direction, speed);
...
BOOL CBulletManager::firetrace_callback	(collide::rq_result& result, LPVOID params)
{
	...
	float const	air_resistance		= Level().BulletManager().m_fAirResistanceK * bullet.air_resistance;//tatarinrafa:Vlad asked to change
...
bool CBulletManager::process_bullet			(collide::rq_results & storage, SBullet& bullet, u32 const delta_time)
{
	...
	float const	air_resistance	= m_fAirResistanceK * bullet.air_resistance; //tatarinrafa:Vlad asked to change
	bullet.tracer_start_position= bullet.bullet_pos;
...
void CBulletManager::CommitEvents			()	// @ the start of frame
{
	...
	for (u32 _it=0; _it<m_Events.size(); _it++)	{
		...
		case EVENT_REMOVE:
			{
#pragma todo("there was a crash for one of the lines below when alt-tabing back to game.")
	...
	case EVENT_HIT:
		{
			...
			if (_dynamic)	
			{
				// Workaround for actor, because it's real ID is always 0
				CActor* actor = smart_cast<CActor*>(R.O);
				u16 targetId = actor
					? (u16)(-1)
					: R.O->ID();

				E.Repeated = (targetId == E.bullet.targetID);
				// Msg("Dynamic object hit event: %s, id: %d, bullet target: %d, repeated: %d", R.O->cName().c_str(), targetId, E.bullet.targetID, E.Repeated);
