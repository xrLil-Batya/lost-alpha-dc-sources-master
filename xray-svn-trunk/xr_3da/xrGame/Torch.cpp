...
void CTorch::Load(LPCSTR section) 
{
	...
	if (Settings_from_ltx == 1){
		...
		lanim = LALib.FindItem(READ_IF_EXISTS(pSettings, r_string, section, "color_animator", "nill"));
		guid_bone = K->LL_BoneID(pSettings->r_string(section, "guide_bone"));

		Fcolor clr = pSettings->r_fcolor(section, (b_r2) ? "color_r2" : "color");
		fBrightness = clr.intensity();
		m_RangeMax = pSettings->r_float(section, (b_r2) ? "range_max_r2" : "range_max");
		m_RangeMin = pSettings->r_float(section, (b_r2) ? "range_min_r2" : "range_min");

		light_render->set_color(clr);
		light_render->set_range(m_RangeMax);

		Fcolor clr_o = pSettings->r_fcolor(section, (b_r2) ? "omni_color_r2" : "omni_color");
		float range_o = pSettings->r_float(section, (b_r2) ? "omni_range_r2" : "omni_range");
		light_omni->set_color(clr_o);
		light_omni->set_range(range_o);

		light_render->set_cone(deg2rad(pSettings->r_float(section, "spot_angle")));
		light_render->set_texture(pSettings->r_string(section, "spot_texture"));

		glow_render->set_texture(pSettings->r_string(section, "glow_texture"));
		glow_render->set_color(clr);
		glow_render->set_radius(pSettings->r_float(section, "glow_radius"));
	}
...
void CTorch::UpdateBattery(void)
{
	if (m_switched_on)
	{
		...
		float range = m_battery_state;//m_current_battery_state;
