...
void			CAI_Stalker::Hit					(SHit* pHDS)
{
	...
	if(m_boneHitProtection && HDS.hit_type == ALife::eHitTypeFireWound)
	{
		...
		Msg("Fire wound, hit_power = %.2f (%.2f), boneArmor = %.2f (bone %d), ap = %.2f", hit_power, HDS.power, BoneArmor, HDS.bone(), ap);
	}
	HDS.power = hit_power;
	...
	if (g_Alive()) {
		...
		if (!already_critically_wounded) {
			...
			if ( !invulnerable() && cover && pHDS->initiator() && (pHDS->initiator()->ID() != ID()) && !fis_zero(pHDS->damage()) && brain().affect_cover())
...
void CAI_Stalker::on_weapon_shot_start		(CWeapon *weapon)
{
	...
	weapon_shot_effector().Shot(weapon->zoom_cam_recoil.camDispersion + weapon->zoom_cam_recoil.camDispersionInc*float(weapon->ShotsFired()));
}
