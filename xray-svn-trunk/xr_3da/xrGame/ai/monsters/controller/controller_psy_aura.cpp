...
: inherited(ppi)
{
	...
	//tatarinrafa: Lets use a bug with r2_aa on dx10 and 11 as a cool effect
	mblur = Console->GetBool("r2_mblur_enable");
	Console->Execute("r2_aa on");
	Console->Execute("r2_mblur_enable off");
}
...
CPPEffectorControllerAura::~CPPEffectorControllerAura(){
	...
	if (mblur){
		Console->Execute("r2_mblur_enable on");
	}
}
