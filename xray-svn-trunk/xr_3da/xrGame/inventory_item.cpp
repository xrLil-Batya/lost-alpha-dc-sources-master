...
{
	if(!m_net_updateData) 
		m_net_updateData = new net_updateData();
...
//.	NameComplex			();
	...
	m_SectNightVision		= READ_IF_EXISTS(pSettings, r_string, section, "nightvision_sect", NULL);
...
void CInventoryItem::net_Destroy		()
{
	if(m_pCurrentInventory){
		VERIFY(std::find(m_pCurrentInventory->m_all.begin(), m_pCurrentInventory->m_all.end(), this) == m_pCurrentInventory->m_all.end());
	}
...
shared_str CInventoryItem::GetUpgrIconName() const
{
	return shared_str(READ_IF_EXISTS(pSettings, r_string, m_object->cNameSect(), "upgr_icon", ""));
}
