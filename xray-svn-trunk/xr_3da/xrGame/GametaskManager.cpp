...
CGameTask* CGameTaskManager::HasGameTask(const CMapLocation* ml, bool only_inprocess)
{
	...
	for(; it!=it_e; ++it)
	{
		...
		for(u16 i=0; i<gt->m_Objectives.size() ;++i)
		{
			SGameTaskObjective& obj = gt->Objective(i);
			if(obj.LinkedMapLocation()==ml)
			{
				if(only_inprocess && obj.TaskState()!=eTaskStateInProgress)
					continue;
...
CGameTask*	CGameTaskManager::GiveGameTaskToActor(CGameTask* t, u32 timeToComplete, bool bCheckExisting)
{
	...
	if(CurrentGameUI()){
		...
		if(pGameSP) 
			pGameSP->m_PdaMenu->PdaContentsChanged	(pda_section::quests);
	}
...
void CGameTaskManager::SetTaskState(CGameTask* t, u16 objective_num, ETaskState state)
{
	...
	if (0 == objective_num && eTaskStateCompleted == state || eTaskStateFail == state || eTaskStateSkiped == state)
	...
	for( ;it!=it_e; ++it )
	{
		...
		for(u32 i=0; i<t->m_Objectives.size() ;++i)
		{
				...
				(t->Objective(i-1).TaskState()==eTaskStateCompleted) )
			{
				...
				{
					CMapLocation* ml			= obj.LinkedMapLocation();
					if(!ml)	ml					= Level().MapManager().AddMapLocation(obj.map_location, obj.object_id);
					if(obj.map_hint.size())		ml->SetHint(obj.map_hint);
					ml->DisablePointer			();
					ml->SetSerializable			(true);
				}
