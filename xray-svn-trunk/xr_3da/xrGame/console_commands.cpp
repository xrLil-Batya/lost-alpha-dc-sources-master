	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (!xr_strcmp(str, "1")){
			if (!pGameSP->m_WeatherEditor->IsShown()){
				...
				//Msg("---Start_ui TIME = %u %u, target weather section = %s", pGameSP->m_WeatherEditor->HoursAtStartUp, pGameSP->m_WeatherEditor->MinsAtStartUp, g_pGamePersistent->Environment().Current[0]->m_identifier.c_str());
			}
		}
...
class CCC_ReloadConfigAndEnvironment : public IConsole_Command
{
public:
	CCC_ReloadConfigAndEnvironment(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = TRUE;
	}
	virtual void Execute(LPCSTR args) {
...
public:
	...
	virtual void Execute(LPCSTR args) {
		...
		/*if (pSettings->section_exist("lost_alpha_cfg") && pSettings->line_exist("lost_alpha_cfg","on_save_callback"))
		{
			...
			xr_strcpy(func_name,pSettings->r_string("lost_alpha_cfg","on_save_callback"));
...
//.		sscanf						(args,"%s",saved_game);
		...
		/*if (pSettings->section_exist("lost_alpha_cfg") && pSettings->line_exist("lost_alpha_cfg","on_load_callback"))
		{
			string256					func_name;
			luabind::functor<LPCSTR>			func;
			xr_strcpy(func_name,pSettings->r_string("lost_alpha_cfg","on_load_callback"));
			R_ASSERT					(ai().script_engine().functor<LPCSTR>(func_name,func));
			func						();
		}*/
...
int				ps_intro = 0;
extern	BOOL	r__skip_texture_load = 0; //tatarinraf: useful for faster r2 3 4 debugging
...
//	CMD4(CCC_Integer,			"hud_adjust_item_index",	&hud_adj_item_idx,	0, 1);
	CMD4(CCC_Float,				"hud_adjust_delta_pos",		&hud_adj_delta_pos,	0.0005f, 1.f);
	CMD4(CCC_Float,				"hud_adjust_delta_rot",		&hud_adj_delta_rot,	0.05f, 10.f);
#endif //DRENDER
	...
	CMD1(CCC_ReloadConfigAndEnvironment,	"cfg_weather_restart");
	...
	CMD4(CCC_Integer,	"r__skip_texture_load",		&r__skip_texture_load,				0, 1);
