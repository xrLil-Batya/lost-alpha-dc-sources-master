...
bool CScriptGameObject::HasUpgrade		(LPCSTR upgrade_id)
{
	CInventoryItem*	inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item)
	{
		ai().script_engine().script_log		(ScriptStorage::eLuaMessageTypeError,"CScriptGameObject::HasUpgrade non-CInventoryItem object !!!");
		return false;
	}

	return inventory_item->has_upgrade(upgrade_id);
}
...
void CScriptGameObject::PlayAnim(LPCSTR anim, BOOL cycle, u32 slot)
{
	...
	hud->PlayHUDMotion	(anim, cycle, hud, hud->GetState());
}
