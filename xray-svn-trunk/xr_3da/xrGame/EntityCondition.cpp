...
float CEntityCondition::HitOutfitEffect(float hit_power, ALife::EHitType hit_type, s16 element, float ap, bool& add_wound)
{
	...
	//Msg("HitOutfitEffect before: hit_power = %f | element(bone_id) = %i", hit_power, element);
	float new_hit_power = hit_power;
	if (pOutfit)
		new_hit_power = pOutfit->HitThroughArmor(hit_power, element, ap, add_wound, hit_type);
	//	Msg("HitOutfitEffect after outfit: hit_power = %f", new_hit_power);
	if (pHelmet)
		new_hit_power = pHelmet->HitThroughArmor(new_hit_power, element, ap, add_wound, hit_type);
	//	Msg("HitOutfitEffect after helmet: hit_power = %f", new_hit_power);
	//if (bDebug)
	//Msg("HitOutfitEffect total: hit_power = %.3f | hit_type = %s  ap = %.3f", new_hit_power, ALife::g_cafHitType2String(hit_type), ap);
...
CWound* CEntityCondition::ConditionHit(SHit* pHDS)
{
	...
	hit_power = HitOutfitEffect(hit_power_org, pHDS->hit_type, pHDS->boneID, pHDS->ap, bAddWound);
