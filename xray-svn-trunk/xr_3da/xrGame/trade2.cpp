...
u32	CTrade::GetItemPrice(PIItem pItem, bool b_buying)
{
	...
	if (is_actor) {
		...
		{
			CActor			*pActor = smart_cast<CActor*>(pThis.inv_owner);
			if (pActor)
				actor_factor = pActor->GetPriceFactor();
		} else if (pPartner.type == TT_ACTOR) {
			CActor			*pActor = smart_cast<CActor*>(pPartner.inv_owner);
			if (pActor)
				actor_factor = pActor->GetPriceFactor();
		}
		if (buying)
			actor_factor = 1.f/actor_factor;
	} else {
		// rare case
		_partner			= &pPartner;
	}
