	  ...
	  m_interactive_motion(0)
{
	...
	m_Pred_Time						= 0.0;
	m_was_wounded					= false;
	m_death_impulse_vec.set(0.f, 0.f, 0.f);
	m_death_impulse_cooldown_speed = 1.f;
	switch(atype)
	{
	case etActor:
...
void CCharacterPhysicsSupport::in_Load(LPCSTR section)
{
	...
	// death impulse accumulation
	m_death_impulse_cooldown_speed = READ_IF_EXISTS(pSettings, r_float, section, "ph_death_impulse_cooldown_speed", 1000.f);
	//lost alpha end
...
void CCharacterPhysicsSupport::in_Hit(float P,Fvector &dir, CObject *who,s16 element,Fvector p_in_object_space, float impulse,ALife::EHitType hit_type ,bool is_killing)
{
	...
	if (!m_pPhysicsShell || !m_pPhysicsShell->isActive() || isKillHit)
	{
		// accumulate impulse
		Fvector3 newImpulse = Fvector3(dir).normalize().mul(impulse);
		m_death_impulse_vec.add(newImpulse);
		//Msg("Accumulate impulse %.2f (%.2f, %.2f, %.2f), result %.f (%.2f, %.2f, %.2f)",
		//	impulse, dir.x, dir.y, dir.z, m_death_impulse_vec.magnitude(), m_death_impulse_vec.x, m_death_impulse_vec.y, m_death_impulse_vec.z);
	}
	if(!(m_pPhysicsShell&&m_pPhysicsShell->isActive()))
	{
		...
		float impulseMagnitude = isKillHit
			? m_death_impulse_vec.magnitude()
			: impulse;

		Fvector3 impulseDir = isKillHit && !fis_zero(impulseMagnitude)
			? Fvector3(m_death_impulse_vec).normalize()
			: dir;

		m_pPhysicsShell->applyHit(p_in_object_space,impulseDir,impulseMagnitude,element,hit_type);
		//Msg("Apply hit to shell (%.2f, %.2f, %.2f) impulse %.2f, kill hit: %d", impulseDir.x, impulseDir.y, impulseDir.z, impulseMagnitude, isKillHit);
	}
}
...
void CCharacterPhysicsSupport::in_UpdateCL( )
{
	...
	if (!fis_zero(m_death_impulse_vec.magnitude())) {
		m_death_impulse_vec.set_length(m_death_impulse_vec.magnitude() - m_death_impulse_cooldown_speed * m_time_delta);
	}
