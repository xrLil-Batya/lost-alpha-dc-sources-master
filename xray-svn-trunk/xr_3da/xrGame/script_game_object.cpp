...
BIND_FUNCTION10	(&object(), CScriptGameObject::GetThirsty,			CEntityAlive,	conditions().GetThirsty,			float,						-1);
...
//---ƒобавил возможность обращатьс€ к кровотечению
...
void CScriptGameObject::TeleportEntity (Fvector pos, Fvector dir)
{
	...
	if (!pAlive || !pAlive->g_Alive())		return;
...
float CScriptGameObject::GetActorMaxWeight ()
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
...
void CScriptGameObject::SetActorMaxWeight (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
	...
	actor->inventory().SetMaxWeight(value);
}

float CScriptGameObject::GetActorWalkWeight ()
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member GetActorWalkWeight!");
		return -1.f;
	}
	return actor->conditions().GetWalkWeight();
}


void CScriptGameObject::SetActorWalkWeight (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member SetActorWalkWeight!");
		return;
	}
	actor->conditions().ChangeWalkWeight(value);
}

void CScriptGameObject::SetActorHitImmunity (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member SetActorHitImmunity!");
		return;
	}
	actor->SetImmunityCoeff(value);
}

void CScriptGameObject::SetActorCamDispersionCoeff (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member SetActorCamDispersionCoeff!");
		return;
	}
	actor->SetCamDispersionCoeff(value);
}

void CScriptGameObject::SetActorZoomInertCoeff (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member SetActorZoomInertCoeff!");
		return;
	}
	actor->SetZoomInertCoeff(value);
}

void CScriptGameObject::SetActorPriceFactor (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member SetActorPriceFactor!");
		return;
	}
	actor->SetPriceFactor(value);
}

void CScriptGameObject::SetActorSprintFactor (float value)
{
	CActor* actor = smart_cast<CActor*>(&object());
	if (!actor) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CActor : cannot access class member SetActorSprintFactor!");
		return;
	}
	actor->SetSprintFactor(value);
}