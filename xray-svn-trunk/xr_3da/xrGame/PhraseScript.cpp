...
bool CPhraseScript::Precondition(const CGameObject* pSpeakerGO, LPCSTR dialog_id, LPCSTR phrase_id) const 
{
	...
	for(u32 i = 0; i<Preconditions().size(); ++i)
	{
		...
		//tatarinrafa: added availability to return reversed precondition result; for script optimization
		bool functor_exists;
		if (strstr(*Preconditions()[i], ".ReturnReverse")){

			xr_string preconditionScript = *Preconditions()[i];
			preconditionScript.erase(preconditionScript.size() - 14);
			Msg("single param %s precondition Reversed ", preconditionScript.c_str());

			functor_exists = ai().script_engine().functor(*Preconditions()[i], lua_function);
										...
										LPCSTR next_phrase_id) const 
{
	...
	for(u32 i = 0; i<Preconditions().size(); ++i)
	{
		...
		if (strstr(*Preconditions()[i], ".ReturnReverse")){
			...
			xr_string preconditionScript = *Preconditions()[i];
			preconditionScript.erase(preconditionScript.size() - 14);
			Msg("double param %s precondition Reversed ", preconditionScript.c_str());

			functor_exists = ai().script_engine().functor(preconditionScript.c_str(), lua_function);
