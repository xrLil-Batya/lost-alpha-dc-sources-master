...
public:
	...
	void					SetCurrentDetector	(CCustomDetectorR * detector) { CurrentDetectorInHand = detector;}; 	// Для детекторов. Использовал для возвращения детектора в руки гг после смены камеры
	CCustomDetectorR *		CurrentDetector		() const { return CurrentDetectorInHand; };
	void					Setneedtoactivateweapon(u32 slottoactivate)	{ needtoactivateweapon = slottoactivate; } //для правильной смены оружия
...
protected:
	...
	u32					needtoactivateweapon; //для правильной смены оружия
