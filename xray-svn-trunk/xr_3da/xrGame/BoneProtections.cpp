...
void SBoneProtections::reload(const shared_str& bone_sect, IKinematics* kinematics)
{
	...
	m_fHitFracNpc = READ_IF_EXISTS(pSettings, r_float, bone_sect, "hit_fraction",	0.1f);
...
void SBoneProtections::add(const shared_str& bone_sect, IKinematics* kinematics)
{
	...
	VERIFY(kinematics);
	m_fHitFracNpc += READ_IF_EXISTS(pSettings, r_float, bone_sect.c_str(), "hit_fraction", 0.0f);
