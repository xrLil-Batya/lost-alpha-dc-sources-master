...
{
	R_ASSERT						( pDialog && !pDialog->IsShown() );
...
#pragma todo("tatarinrafa:ћожет закомнетить эту хрень?|May be we dont even need this?")
				//if (action != kDETECTOR) // Ќе могу пон€ть почему вылетает именно детектор и зачем вообще этот вторичный вызов нужен
