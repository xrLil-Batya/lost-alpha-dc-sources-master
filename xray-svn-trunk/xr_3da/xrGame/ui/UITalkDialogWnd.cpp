...
void CUITalkDialogWnd::ShowTradeButton(bool can_trade, bool can_upgrade)
{
	UIToTradeButton.Show(can_trade || can_upgrade);
	UIToTradeButton.SetTextST(can_upgrade ? "ui_st_repair_upgr" : "ui_st_trade");
}
...
{
	m_text							= new CUI3tButton();m_text->SetAutoDelete(true);
	AttachChild						(m_text);
