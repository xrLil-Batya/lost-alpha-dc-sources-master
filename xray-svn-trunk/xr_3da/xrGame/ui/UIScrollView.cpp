...
void CUIScrollView::InitScrollView				()
{
	...
	if (!m_VScrollBar)
	{
		...
		AddCallbackStr					("scroll_v",	SCROLLBAR_VSCROLL, CUIWndCallback::void_function (this, &CUIScrollView::OnScrollV) );
	}
...
void CUIScrollView::SetSelected			(CUIWindow* w)
{
	...
	if (m_pad)
		{
		if (m_pad->GetChildNum() > 0)
			{
			for (WINDOW_LIST_it it = m_pad->GetChildWndList().begin(); it != m_pad->GetChildWndList().end(); ++it)
				{
