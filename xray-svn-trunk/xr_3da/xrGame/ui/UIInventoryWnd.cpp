...
void CUIInventoryWnd::Init()
{
	...
	m_pUIOutfitList						= new CUIDragDropListEx(); AttachChild(m_pUIOutfitList); m_pUIOutfitList->SetAutoDelete(true);
	...
	m_pUIKnifeList						= new CUIDragDropListEx(); AttachChild(m_pUIKnifeList); m_pUIKnifeList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_knife", 0, m_pUIKnifeList);
	BindDragDropListEvents				(m_pUIKnifeList);

	m_pUIBinocularList						= new CUIDragDropListEx(); AttachChild(m_pUIBinocularList); m_pUIBinocularList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_binocular", 0, m_pUIBinocularList);
	BindDragDropListEvents				(m_pUIBinocularList);

	m_pUITorchList						= new CUIDragDropListEx(); AttachChild(m_pUITorchList); m_pUITorchList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_torch", 0, m_pUITorchList);
	BindDragDropListEvents				(m_pUITorchList);

	m_pUIDetectorList						= new CUIDragDropListEx(); AttachChild(m_pUIDetectorList); m_pUIDetectorList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_detecor", 0, m_pUIDetectorList);
	BindDragDropListEvents				(m_pUIDetectorList);

	m_pUIHelmetList						= new CUIDragDropListEx(); AttachChild(m_pUIHelmetList); m_pUIHelmetList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_helmets", 0, m_pUIHelmetList);
	BindDragDropListEvents				(m_pUIHelmetList);

	m_pUIPNVList						= new CUIDragDropListEx(); AttachChild(m_pUIPNVList); m_pUIPNVList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_pnv", 0, m_pUIPNVList);
	BindDragDropListEvents				(m_pUIPNVList);

	m_pUIAnomDetectorList						= new CUIDragDropListEx(); AttachChild(m_pUIAnomDetectorList); m_pUIAnomDetectorList->SetAutoDelete(true);
	xml_init.InitDragDropListEx			(uiXml, "dragdrop_anomaly_detector", 0, m_pUIAnomDetectorList);
	BindDragDropListEvents				(m_pUIAnomDetectorList);
