...
void CUIInventoryUpgradeWnd::InitInventory( CInventoryItem* item, bool can_upgrade )
{
	...
	bool display_icon = smart_cast<CWeapon*>(item) || smart_cast<CCustomOutfit*>(item) || smart_cast<CCustomDetectorR*>(item);

	if (m_item && display_icon)
	{
		shared_str item_upgrade_icon = item->GetUpgrIconName();
		Irect item_upgrade_grid_rect = item->GetUpgrIconRect();
		Frect texture_rect;

		m_item->InitTexture(*item_upgrade_icon);
...
//tarinrafa: Added Clear function to avoid upgrading with folowing crash of game
void CUIInventoryUpgradeWnd::Clear()
{
	m_item->Show		(false);
	m_scheme_wnd->DetachAll();
	m_scheme_wnd->Show( false );
...
void CUIInventoryUpgradeWnd::Hide()
{
	Show(false);
}
...
void CUIInventoryUpgradeWnd::SwitchToTalk()
{
...
void CUIInventoryUpgradeWnd::AskUsing( LPCSTR text, LPCSTR upgrade_name )
{
	...
	CUIUpgradeWnd* parent_wnd = smart_cast<CUIUpgradeWnd*>(m_pParentWnd);
	if ( parent_wnd )
	{
...
void CUIInventoryUpgradeWnd::set_info_cur_upgrade( Upgrade_type* upgrade )
{
	...
	CUIUpgradeWnd* parent_wnd = smart_cast<CUIUpgradeWnd*>(m_pParentWnd);
	if (parent_wnd && parent_wnd->SetInfoCurUpgrade(upgrade, m_inv_item))
		UpdateAllUpgrades();
}
