...
void CUITradeWnd::MarkAmmo(CUICellItem* itm)
{
	...
	xr_vector<shared_str> scope_types;
	xr_vector<shared_str> silencer_types;
	xr_vector<shared_str> podstvol_types;
	if (weapon){
		// Подгрузим список подходящих (если)прицелов
		scope_types.clear();

		LPCSTR	S				= READ_IF_EXISTS(pSettings, r_string, inventoryitem->object().cNameSect(), "scope_name", "null");
		u32		notanAddon		= READ_IF_EXISTS(pSettings, r_u32, inventoryitem->object().cNameSect(), "scope_status", 0);
		if (S && S[0] && notanAddon != 1)
		{
			string128		_addon;
			int				count = _GetItemCount(S);
			for (int it = 0; it < count; ++it)
			{
				_GetItem(S, it, _addon);
				scope_types.push_back(_addon);
			}
		}


		// Подгрузим список подходящих (если)глишителей
		silencer_types.clear();
		S = READ_IF_EXISTS(pSettings, r_string, inventoryitem->object().cNameSect(), "silencer_name", "null");
		notanAddon = READ_IF_EXISTS(pSettings, r_u32, inventoryitem->object().cNameSect(), "silencer_status", 0);
		if (S && S[0] && notanAddon != 1)
		{
			string128		_addon;
			int				count = _GetItemCount(S);
			...
			{
				_GetItem(S, it, _addon);
				silencer_types.push_back(_addon);
			}
		}


		// Подгрузим список подходящих (если)подстволов
		podstvol_types.clear();
		S = READ_IF_EXISTS(pSettings, r_string, inventoryitem->object().cNameSect(), "grenade_launcher_name", "null");
		notanAddon = READ_IF_EXISTS(pSettings, r_u32, inventoryitem->object().cNameSect(), "grenade_launcher_status", 0);
		if (S && S[0] && notanAddon != 1)
		{
			string128		_addon;
			int				count = _GetItemCount(S);
			...
			{
				_GetItem(S, it, _addon);
				podstvol_types.push_back(_addon);
			}
		}
	}

	u32 item_count = m_uidata->UIOurBagList.ItemsCount();

	for (u32 i = 0; i<item_count; ++i) {
		CUICellItem* cell_item = m_uidata->UIOurBagList.GetItemIdx(i);
		PIItem invitem = (PIItem)cell_item->m_pData;

		cell_item->SetTextureColor(0xffffffff);
		...
		if (weapon){
			...
			for (size_t id = 0; id < ammo_types.size(); ++id) {
				if (invitem && xr_strcmp(invitem->object().cNameSect(), ammo_types[id]) == 0 && invitem->Useful()) {
					cell_item->SetTextureColor(color);
				}
			}
			for (size_t id = 0; id < scope_types.size(); ++id) {
				if (invitem && xr_strcmp(invitem->object().cNameSect(), scope_types[id]) == 0 && invitem->Useful()) {
					cell_item->SetTextureColor(color_rgba(255, 150, 50, 255));
				}
			}
			for (size_t id = 0; id < silencer_types.size(); ++id) {
				if (invitem && xr_strcmp(invitem->object().cNameSect(), silencer_types[id]) == 0 && invitem->Useful()) {
					cell_item->SetTextureColor(color_rgba(255, 150, 50, 255));
				}
			}
			for (size_t id = 0; id < podstvol_types.size(); ++id) {
				if (invitem && xr_strcmp(invitem->object().cNameSect(), podstvol_types[id]) == 0 && invitem->Useful()) {
					cell_item->SetTextureColor(color_rgba(255, 150, 50, 255));
				}
			}
		}
	...
	for (u32 i = 0; i<item_count; ++i) {
		CUICellItem* cell_item = m_uidata->UIOthersBagList.GetItemIdx(i);
		PIItem invitem = (PIItem)cell_item->m_pData;
	...
	for (u32 i = 0; i<item_count; ++i) {
		CUICellItem* cell_item = m_uidata->UIOurTradeList.GetItemIdx(i);
		PIItem invitem = (PIItem)cell_item->m_pData;
	...
	for (u32 i = 0; i<item_count; ++i) {
		CUICellItem* cell_item = m_uidata->UIOthersTradeList.GetItemIdx(i);
		PIItem invitem = (PIItem)cell_item->m_pData;
