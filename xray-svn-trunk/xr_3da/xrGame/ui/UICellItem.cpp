...
CUICellItem::CUICellItem()
{
...

	{
		...
		m_unique_mark = new CUIStatic();
		m_unique_mark->SetAutoDelete(true);
		AttachChild(m_unique_mark);
		CUIXmlInit::InitStatic(uiXml, "cell_item_uniq_mark", 0, m_unique_mark);
		m_unique_mark_pos = m_unique_mark->GetWndPos();
		m_unique_mark->Show(false);
	}
	
	{
		...
		m_quest_mark = new CUIStatic();
		m_quest_mark->SetAutoDelete(true);
		AttachChild(m_quest_mark);
		CUIXmlInit::InitStatic(uiXml, "cell_item_quest_mark", 0, m_quest_mark);
		m_quest_mark_pos = m_quest_mark->GetWndPos();
		m_quest_mark->Show(false);
	}
}
...
void CUICellItem::Update()
{
	...
	if ( item )
	{
		...
		is_unique = item->IsUniqueMarked();
		if (item->IsQuestItem())is_quest = true;

//		Fvector2 size      = GetWndSize();
//		Fvector2 up_size = m_upgrade->GetWndSize();
//		pos.x = size.x - up_size.x - 4.0f;
		Fvector2 pos, poss, posss;

		pos.set(m_unique_mark_pos );
		poss.set(m_quest_mark_pos);
		posss.set(m_upgrade_pos);

		float w = Device.dwWidth;
		float h = Device.dwHeight;

		if (w / h > 1.5f){ // if 16:9
			pos.x += GetWndSize().x - 10.0;
			poss.x += GetWndSize().x - 10.0;
			posss.x += GetWndSize().x - 10.0;
			if (is_unique) {
				poss.x -= 10.0;
				posss.x -= 10.0;
			}
			...
			pos.x += GetWndSize().x - 12.0;
			poss.x += GetWndSize().x - 12.0;
			posss.x += GetWndSize().x - 12.0;
			if (is_unique) {
				poss.x -= 12.0f;
				posss.x -= 12.0f;
			}
		...
		pos.y += GetWndSize().y - 12.0;
		poss.y += GetWndSize().y - 12.0;
		posss.y += GetWndSize().y - 12.0;

		//if ( ChildsCount() )
		//{
			//pos.x += m_text->GetWndSize().x + 1.0f;
			//poss.x += m_text->GetWndSize().x + 1.0f;
			//posss.x += m_text->GetWndSize().x + 1.0f;

		//}

		m_unique_mark->SetWndPos	(pos);
		m_quest_mark->SetWndPos		(poss);
		m_upgrade->SetWndPos		(posss);
	}
	m_upgrade->Show		( m_has_upgrade );
	m_unique_mark->Show	( is_unique );
	m_quest_mark->Show	( is_quest );
}
