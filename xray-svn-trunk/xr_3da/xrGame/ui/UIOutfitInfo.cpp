...
void CUIOutfitInfo::Update(CCustomOutfit* outfit, CHelmet* helmet)
{
	m_outfit				= outfit;
	m_helmet				= helmet;
...
{
	string128 _buff;
	float _val_booster	= 0.0f;
	float _val_outfit	= 0.0f;
	float _val_af		= 0.0f;
	float _val_helmet	= 0.0f;

	CUIStatic* _s		= m_items[hitType];

	_val_outfit			= m_outfit ? m_outfit->GetDefHitTypeProtection(ALife::EHitType(hitType)) : 0.0f;
	_val_outfit			= _val_outfit;


	_val_helmet			= m_helmet ? m_helmet->GetDefHitTypeProtection(ALife::EHitType(hitType)) : 0.0f;
	_val_helmet			= _val_helmet;//1.0f - _val_helmet;
	...
	_val_booster		= Actor()->HitBoosters(1.0f,ALife::EHitType(hitType));
	_val_booster		= 1.0f - _val_booster;


	if (fsimilar(_val_outfit, 0.0f) && fsimilar(_val_af, 0.0f) && fsimilar(_val_booster, 0.0f) && fsimilar(_val_helmet, 0.0f) && !force_add)
	{
...
//	LPCSTR			_clr_outfit, _clr_af;
	...
	if (!fsimilar(_val_helmet, 0.0f))
	{
		_sz += xr_sprintf(_buff + _sz, sizeof(_buff) - _sz, "%s %+3.0f%%", (_val_helmet>0.0f) ? "%c[green]" : "%c[red]", _val_helmet*100.0f);
	}
	...
	if (!fsimilar(_val_booster, 0.0f))
	{
		_sz += xr_sprintf(_buff + _sz, sizeof(_buff) - _sz, "%s %+3.0f%%", (_val_booster>=0.0f) ? "%c[ui_pda_orange]" : "%c[ui_pda_violet]", _val_booster*100.0f);
	}
