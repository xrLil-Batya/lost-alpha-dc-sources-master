...
float CalcMotionSpeed(const shared_str& anim_name)
{
	...
	if(!IsGameTypeSingle() && (anim_name=="anim_show" || anim_name=="anim_hide") )
		return 2.0f;
	else
		return 1.0f;
}
...
void player_hud_motion_container::load(IKinematicsAnimated* model, const shared_str& sect)
{
	...
	for(;_b!=_e;++_b)
	{
		if(strstr(_b->first.c_str(), "anim_")==_b->first.c_str())
		{
...
u32 attachable_hud_item::anim_play(const shared_str& anm_name_b, BOOL bMixIn, const CMotionDef*& md, u8& rnd_idx)
{
	...
	R_ASSERT				(strstr(anm_name_b.c_str(),"anim_")==anm_name_b.c_str());
	...
	{
		CActor* current_actor	= smart_cast<CActor*>(Level().CurrentViewEntity());
		if (current_actor)
		{
...
void player_hud::update(const Fmatrix& cam_trans)
{
	...
	if (bobbing_enabled() && psActorFlags.test(AF_WPN_BOBBING))
		m_bobbing->Update(m_attach_offset);

	bool bCollide = collide_enabled();
	bool bStrafes = strafes_enabled() && psActorFlags.test(AF_STRAFE_INERT);
	if (bCollide || bStrafes)
	{ 
		m_collision->CheckState();

		if (bCollide)
		{
			collide::rq_result& RQ = HUD().GetCurrentRayQuery();
			m_collision->UpdateCollision(m_attach_offset, RQ.range);
		} else
			m_collision->UpdateStrafes(m_attach_offset);
	}
...
bool player_hud::inertion_allowed()
{
	...
	if(hi)
		return hi->m_parent_hud_item->HudInertionAllowed()==1?true:false;
	else
		return true;
}

bool player_hud::inertion_enabled()
{
	attachable_hud_item* hi = m_attached_items[0];
	if(hi)
		return hi->m_parent_hud_item->HudInertionEnabled()==1?true:false;
	else
		return true;
}

bool player_hud::collide_enabled()
{
	attachable_hud_item* hi1 = m_attached_items[0];
	attachable_hud_item* hi2 = m_attached_items[1];
	return ((hi1 && hi1->m_parent_hud_item->HudCollideEnabled()) || (hi2 && hi2->m_parent_hud_item->HudCollideEnabled()));
}

bool player_hud::strafes_enabled()
{
	attachable_hud_item* hi1 = m_attached_items[0];
	attachable_hud_item* hi2 = m_attached_items[1];
	return ((hi1 && hi1->m_parent_hud_item->HudStrafeInertionEnabled()) || (hi2 && hi2->m_parent_hud_item->HudStrafeInertionEnabled()));
}

bool player_hud::bobbing_enabled()
{
	attachable_hud_item* hi1 = m_attached_items[0];
	attachable_hud_item* hi2 = m_attached_items[1];
	return ((hi1 && hi1->m_parent_hud_item->HudBobbingEnabled()) || (hi2 && hi2->m_parent_hud_item->HudBobbingEnabled()));
}
