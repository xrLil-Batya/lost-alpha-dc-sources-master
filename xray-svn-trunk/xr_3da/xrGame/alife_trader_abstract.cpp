...
#pragma warning(pop)
		...
		if (specific_character_supply)
		{
			...
			SSpecificCharacterSupplies sup = selected_char.Supplies();
			spawn_supplies(sup);
...
static void SpawnItem(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies::Item& defn)
{
	for (u32 i = 0; i < defn.count; ++i)
	{
		if (::Random.randF(1.f) < defn.probability)
		{
			auto item = smart_cast<CSE_ALifeInventoryItem*>(
				dynamic_object->spawn_item(
					defn.section.c_str(),
					defn.condition,
					defn.hasScope,
					defn.hasLauncher,
			...
			if (item)
			{
				// Pre-installed upgrades for weapons and outfits
				for (auto& upgr : defn.upgrades)
				{
					item->add_upgrade(upgr);
				}
			}
			CSE_ALifeItemWeapon* W = smart_cast<CSE_ALifeItemWeapon*>(item);
			if (W)
			{
				if (!defn.ammoType.empty())
				{
					int ammoNum = _GetItemCount(W->m_caAmmoSections);
					for (int j = 0; j < ammoNum; j++)
					{
						string128 sAmmo;
						_GetItem(W->m_caAmmoSections, j, sAmmo);
						if (xr_strcmp(defn.ammoType.c_str(), sAmmo) == 0)
						{
							W->set_ammo_type(j);
							break;
						}
				...
				if (defn.ammoCount >= 0)
				{
					W->set_ammo_elapsed(_min(W->get_ammo_magsize(), defn.ammoCount));
				}
			}
		}
	}
...
void CSE_ALifeTraderAbstract::spawn_supplies(const SSpecificCharacterSupplies& supplies)
{
	static int nestLevel = 0;
	R_ASSERT(nestLevel < 30);
	CSE_ALifeDynamicObject *dynamic_object = smart_cast<CSE_ALifeDynamicObject*>(this);
	if (!supplies.m_templates.empty())
	{
		for (auto& tmplRef : supplies.m_templates)
		{
			CSpecificCharacterSupplyTemplate tmpl;
			tmpl.Load(tmplRef.name);
			++nestLevel;
			spawn_supplies(tmpl.Supplies());
			--nestLevel;
		}
	}
	if (!supplies.m_items.empty())
	{
		for (auto& item : supplies.m_items)
		{
			SpawnItem(dynamic_object, item);
		}
	}
}
