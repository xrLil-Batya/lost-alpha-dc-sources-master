...
//	else { m_zoom_params.m_fScopeZoomFactor = 0; }
	...
	//tatarinrafa: Добавил обход вылета для криворуких конфигов/added workarround for crash when there is no section for ammo
	LPCSTR sextion = *m_ammoTypes[m_ammoType];
	if (!sextion){ m_ammoType = 1; Msg("ERROR When net_Spawning weapon %s, Can't work with AmmoType %u, since its section is %s", Name(), E->ammo_type, sextion); };
...
void CWeapon::save(NET_Packet &output_packet)
{
	...
	save_data(m_fRTZoomFactor, output_packet); //tatarinrafa: Должно помочь с бесконечным зумом
}
...
void CWeapon::load(IReader &input_packet)
{
	...
	load_data(m_fRTZoomFactor, input_packet); //tatarinrafa: Должно помочь с бесконечным зумом
}
...
void CWeapon::OnZoomIn()
{
	...
	if (m_zoom_params.m_bUseDynamicZoom){
		if (m_fRTZoomFactor == 0.f){// При съемном прицеле, если еще не задана m_fRTZoomFactor, то взять у m_zoom_params.m_fScopeZoomFactor
