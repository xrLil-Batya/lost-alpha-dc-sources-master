...
#pragma todo("replaced game type check here by truee")
	if (H_Parent() || true || ((NumItems > 1) && OnClient()))
		NumItems = 0;
...
void CActor::save(NET_Packet &output_packet)
{
	...
	output_packet.w_float(float(inventory().GetMaxWeight())); // skyloader: for skills
}
...
bool				CActor::InventoryAllowSprint			()
{
	...
	//tatarinrafa added sprint_allowed to artefacts
	for (int it = 0; it < inventory().m_belt.size(); ++it)
	{
		CArtefact*	artefact = smart_cast<CArtefact*>(inventory().m_belt[it]);
