...
void	CRenderTarget::phase_scene_prepare	()
{
		...
		)
	{
      ...
      if( !RImplementation.o.dx10_msaa )
         HW.pContext->ClearDepthStencilView(HW.pBaseZB, D3D10_CLEAR_DEPTH|D3D10_CLEAR_STENCIL, 1.0f, 0);
      else
      {
		HW.pContext->ClearRenderTargetView(rt_Color->pRT, ColorRGBA);
		HW.pContext->ClearRenderTargetView(rt_Accumulator->pRT, ColorRGBA);
		HW.pContext->ClearDepthStencilView(rt_MSAADepth->pZRT, D3D10_CLEAR_DEPTH|D3D10_CLEAR_STENCIL, 1.0f, 0);
		HW.pContext->ClearDepthStencilView(HW.pBaseZB, D3D10_CLEAR_DEPTH|D3D10_CLEAR_STENCIL, 1.0f, 0);
      }
   }
	else
