...
#ifdef TIXML_USE_STL
void TiXmlElement::SetDoubleAttribute(const xr_string& name, double val)
{
	TiXmlAttribute* attrib = attributeSet.FindOrCreate(name);
	if (attrib) {
		attrib->SetDoubleValue(val);
...
#ifdef TIXML_USE_STL
void TiXmlElement::SetAttribute(const xr_string& _name, const xr_string& _value)
{
...
#ifdef TIXML_USE_STL
TiXmlDocument::TiXmlDocument(const xr_string& documentName) : TiXmlNode(TiXmlNode::TINYXML_DOCUMENT)
{
	tabsize = 4;
	useMicrosoftBOM = false;
	value = documentName;
...
#ifdef TIXML_USE_STL
TiXmlAttribute* TiXmlAttributeSet::Find(const xr_string& name) const
{
...
TiXmlAttribute* TiXmlAttributeSet::FindOrCreate(const xr_string& _name)
{
