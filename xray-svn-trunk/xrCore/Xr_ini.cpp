...
#ifdef KEEP_INHERITED_SECTIONS
					Current->inherited_sections.push_back(tmp);
#endif
				}
...
#ifdef KEEP_INHERITED_SECTIONS
		u32 count = (*r_it)->inherited_sections.size();
		if (count)
		{			
			for (u32 i = 0; i < count; i++)
			{
				xr_strcat(temp, (i) ? "," : ":");
				xr_strcat(temp, (*r_it)->inherited_sections[i].c_str());
			}
		}
#endif
