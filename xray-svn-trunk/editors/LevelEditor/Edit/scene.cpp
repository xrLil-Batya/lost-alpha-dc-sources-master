...
void st_LevelOptions::Reset()
{
    ...
    m_bDeathmatch		= FALSE;
    m_bTeamDeathmatch	= FALSE;
    m_bArtefacthunt		= FALSE;
}
...
void EScene::FillProp(LPCSTR pref, PropItemVec& items, ObjClassID cls_id)
{
    ...
    PHelper().CreateBOOL		(items,PrepareKey(pref,"Scene\\Usage\\Deathmatch"),				&m_LevelOp.m_bDeathmatch);
    PHelper().CreateBOOL		(items,PrepareKey(pref,"Scene\\Usage\\Teamdeathmatch"),			&m_LevelOp.m_bTeamDeathmatch);
    PHelper().CreateBOOL		(items,PrepareKey(pref,"Scene\\Usage\\Artefacthunt"),			&m_LevelOp.m_bArtefacthunt);
